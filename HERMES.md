...
- 资料分析：Russell 不会点到三级。默认广东日练 **4 篇 × 5 题 = 20 题**，`paper_style=gd`。
  ...
  `solver-canon/07-ziliao.md`。禁写 `资料分析-综合分析-综合判断`。不要等他指定槽位。
  4 篇里 3 篇答案是 ABCD 各一 + 1 个重复，1 篇故意打散；先算定、再把正确项排到指定字母。
...
- 资料分析质量下限：Gemini Flash 的实际 prompt 必须包含 `ziliao-paper-styles.md` 的「共用+gd」和 `quality-feedback.md` 的 R001/R005/R006/R007/R009/R016/R017/R018。广东材料默认 4 段、420–650 字；正文不得放“模拟数据”声明、口号或不参与设问的注水句；每篇至少 3 段数据被题目使用；同一整卷不得跨材料复制同一未知量、列式和三组干扰路径。每个错误项须有不同且可复算的错因，带图留出包必须 `--images yes`，320px 展示时文字不小于 12px。任一项不满足就返工，不得过 gate。`generation_gate.py issue` 会由 ExamSystem 自动调用 Gemini Flash 多模态质检全部原图与 320px 考生视图，并把材料、找数侧车和图片哈希写入回执；不依赖 Cursor，也不能靠文字证据绕过。
