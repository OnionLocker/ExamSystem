...
- **录屏/真题复盘必须写**：视频分析本身不落库。带上报告讲解时，对每道有对错的题用 `exam --exam-id <场次id> --item <题号>` 写一次；重复带同一场同一题会 `already recorded`，不要换标签再记。
...
- 资料分析：Russell 不会点到三级。默认广东日练 **4 篇 × 5 题 = 20 题**，`paper_style=gd`。
  国考 / 深圳 / 拔高只在他显式点名时才换。先跑
  `python3 /home/ubuntu/ExamSystem/scripts/learner_snapshot.py --ziliao-pack`：
  按吐出的 4 篇形态、20 个知识库主标签和每题指定答案字母写题。`tags[0]` 必须逐字来自
  `solver-canon/07-ziliao.md`。禁止 `资料分析-综合分析-综合判断`。不要等他指定槽位。
  4 篇里 3 篇答案是 ABCD 各一 + 1 个重复，1 篇故意打散；先算定再把正确项排到指定字母。
...
- Data-analysis tables and charts are not route D. Render them with `scripts/render_ziliao_figure.py` (`table` / `bars` / `pie`) from the same invented, self-consistent numbers used in the B-route check. Never use Gemini/Banana to draw a 资料分析 table, axis, or pie. Default Guangdong batch is 4×5=20: one text-only, one text+table, one text+chart, one mixed with at least one option figure. Among the 4 materials, exactly 3 use ABCD-each-once plus one extra random letter; 1 material is deliberately unbalanced. Put the computed value into the assigned option letter. Raise difficulty or switch to 国考/深圳 only when Russell says so. Feed `quiz-pipeline/references/ziliao-paper-styles.md` to Gemini Flash.
- 资料分析质量下限：Gemini Flash 的实际 prompt 必须包含 `ziliao-paper-styles.md` 的「共性+gd」和 `quality-feedback.md` 的 R001/R005/R006/R007/R009/R016/R017/R018。广东材料默认 4 段、420–650 字；正文不得放“模拟数据”声明、口号或不参与设问的注水句。每篇至少 3 段数据被题目使用；同一整卷不得跨材料复制同一未知量、列式和三组干扰路径。每个错误项须有不同且可复算的错因，带图留出包必须 `--images yes`，320px 展示时文字不小于 12px。任一项不满足就返工，不得绕过 gate。`generation_gate.py issue` 会由 ExamSystem 自动调用 Gemini Flash 多模态质检全部原图与 320px 考生视图，并把材料、找数侧车和图片哈希写入回执；不依赖 Cursor，也不能靠文字证据绕过。
