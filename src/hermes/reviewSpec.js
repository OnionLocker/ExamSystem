...
const sharedLayoutRules = [
  ...
  '2. 原题之后写 `**作答结果**：你的答案 X · 正确答案 Y · 用时 MM:SS · 建议用时 ≤xx–xxs`，有用时则补对照（快/达标/超时）。下一行写 `本题考察知识点：模块-一级-二级`。看图题必须先看题干再看对应 `qN-stem.png`（或报告题图路径）再写解析；图上的数值、接线和结构以图为准，禁止只凭文字补造。题干里的 A/B/C/D 地名或序号必须留在原句，不要拆行。',
