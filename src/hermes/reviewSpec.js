...
export const REVIEW_COACH_RULES = [
  ...
  '骨架：`> **原题**` → `**作答结果**`（对错/用时/建议用时/考点）→ `#### 草稿诊断`（可省略）→ `#### 标准解析` → `#### 考场解法` → `#### 下次动作` → `#### AI 深度点拨` → `#### 智能统计`（轻量对照）。禁止改成「为什么会错 / 解题流程 / 下次遇到怎么做」。',
...
const sharedLayoutRules = [
  ...
  '1. 每一道展开复盘的题：`### 02 · 题型名`；下一块为 `> **原题**`，同一引用块内先完整照录题干（题干里的 A、B、C 地名/序号必须留在原句，禁止拆成单独一行），四个选项只用 `> **A.**` / `> **B.**` / `> **C.**` / `> **D.**` 各占一行。禁止横向表格、禁止四个选项挤在同一行、禁止省略任何选项、禁止在原题区标答案。',
  '2. 原题之后写 `**作答结果**：你的答案 X · 正确答案 Y · 用时 MM:SS · 建议用时 ≤xx–xxs`，有用时则补对照（快/达标/超时）。下一行写 `本题考察知识点：模块-一级-二级`。题目依赖配图时结合实际图片，禁止凭文字补造图形。',
...
export function buildPracticeReviewLead(review = {}) {
  return [
    ...
    '',
    '言语、判断、数量、资料必须同一套标题，禁止按模块换版式。',
    '如果同时附有草稿图片，请把图片与 Markdown 中对应题号、正确性和用时一起分析。除原题外，不要机械复述报告统计或现成解析。报告里的「建议用时」直接采用。',
    ...sharedLayoutRules,
    '',
  ].join('\n');
}
