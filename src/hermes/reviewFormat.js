...
const PROMPT = /(?:由此可以推出|由此可知|可以推出的是|无法推出的是|不能推出的是|根据以上(?:资料|材料)|根据上述(?:资料|材料)|能够从上述资料中推出的是|填入画横线部分最恰当的一项是|应填入画横线部分最恰当的一项是|下列哪个正确|下列(?:说法|选项|分析)?(?:中)?(?:正确|错误|不正确)的是|以下(?:选项)?(?:如果为真，)?最(?:能|不能)[^。：:]{0,24}|最能削弱|最能加强)[:：]?/g;
