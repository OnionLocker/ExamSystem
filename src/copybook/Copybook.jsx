  ...
  return (
    <div className="space-y-8 pb-12">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={generating}
              className="px-6 py-4 rounded-2xl bg-[#2c261c] text-white font-black text-xs uppercase tracking-widest hover:brightness-110 active:scale-95 transition-all flex items-center space-x-2 shadow-lg shadow-black/10 disabled:opacity-50"
            >
              ...
              <button
                ...
                className={`p-4 rounded-2xl border transition-all text-center flex flex-col items-center justify-between space-y-2 ${
                  isSelected
                    ...
                    : 'border-[#e8d5b0] bg-white hover:border-slate-300 text-[#1a1a1a]'
                }`}
              >
                <span className={`text-[10px] font-bold uppercase ${isSelected ? 'text-[#6b5428]' : 'text-slate-400'}`}>
                  {item.pinyin}
                </span>
                <span className="text-3xl font-black font-serif my-1">{item.char}</span>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${isSelected ? 'bg-white/10 text-white/80' : 'bg-[#e8d5b0] text-slate-500'}`}>
                  {item.focus}
                </span>
              </button>
            );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* 左侧要领卡片 */}
        <div className="bg-white rounded-[2rem] border border-[#e8d5b0] p-6 space-y-4 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-black uppercase tracking-widest text-slate-400">书写结构剖析</span>
              <span className="text-2xl font-serif font-black text-[#6b5428]">{selectedChar.char}</span>
            </div>
            <h4 className="text-base font-black italic mb-2 text-[#1a1a1a]">核心重点：{selectedChar.focus}</h4>
            <p className="text-xs font-medium text-slate-600 leading-relaxed bg-[#e8d5b0]/50 p-4 rounded-xl border border-[#e8d5b0]">
              {selectedChar.tip}
            </p>
          </div>

          <div className="space-y-2 text-xs font-bold text-slate-500 border-t border-[#e8d5b0] pt-4">
        ...
        {/* 右侧标准矢量米字格字帖 */}
        <div className="lg:col-span-2 bg-white rounded-[2rem] border border-[#e8d5b0] p-6 flex flex-col items-center justify-center space-y-4 shadow-sm">
      ...
      <div className="bg-white rounded-[2rem] border border-[#e8d5b0] p-8 space-y-6">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            <h3 className="text-lg font-black italic text-[#1a1a1a] flex items-center gap-2">
              <Image size={20} className="text-[#6b5428]" />
          ...
          <div className="flex items-center space-x-2 bg-[#e8d5b0] p-1 rounded-xl">
            <button
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 bg-[#e8d5b0]/50 border border-[#e8d5b0] rounded-xl px-4 py-2.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-[#6b5428]"
            />
            <button
              onClick={handleGenerate}
              className="px-5 py-2.5 bg-[#1a1a1a] text-white rounded-xl text-xs font-black hover:bg-[#2c261c] hover:text-white transition-colors"
            >
              生成测试
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
          {(activeModel === 'all' || activeModel === 'image2') && (
            <div className="border border-[#e8d5b0] rounded-2xl p-5 space-y-4 bg-gradient-to-b from-white to-[#f9f8f6]">
              ...
              <div className="aspect-video bg-[#e8d5b0] rounded-xl flex flex-col items-center justify-center border border-dashed border-slate-300 relative overflow-hidden group">
              ...
              <div className="text-xs font-medium text-slate-600 bg-white p-3 rounded-xl border border-[#e8d5b0] space-y-1">
