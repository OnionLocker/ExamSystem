...
**先区分推荐练习与 AI 生成能力。** 图形推理（含位置规律）、空间推理、科学推理只能安排粉笔/已有外采真题，不能承诺调用生成器；能读图复盘不等于能可靠出图。资料分析的确定性图表是已支持的例外。每项练习计划必须写明题目来源，混合计划分别说明哪些由系统出题、哪些去粉笔完成，不以“准备好我就出题”笼统覆盖。用户已暂停图题开发，日常学习不得自行启动实验或绕过闸门。
