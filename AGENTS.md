...
- **出题 / 质检才调大模型**：`scripts/generation_gate.py` → `scripts/quality_orchestrator.py`（系统质检）、`scripts/eval_flash.py`（真题自测）、`scripts/ziliao_visual_gate.py`（资料分析视觉质检）等，都通过一个 **OpenAI 兼容端点（cliproxy）** 调 Gemini。相关环境变量（默认值）：
  ...
  - **注意 `.env` 加载差异**：`node` 后端由 `--env-file=.env` 自动注入这些变量；但 Python 出题/质检脚本读 `os.environ`、**不会自动读 `.env`**。直接 `python3 scripts/quality_orchestrator.py ...` 会报 key 缺失，运行前需先把变量导出到当前 shell，例如 `set -a; . ./.env; set +a`（或 `env $(grep -v '^#' .env | xargs) python3 ...`）。
