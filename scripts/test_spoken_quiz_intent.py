...
from kaodian_taxonomy import NUM_EXTREME, NUM_PERM_BASIC, NUM_PERM_SPECIAL, TRANSLATION
...
class SpokenQuizIntentTest(unittest.TestCase):
    def test_special_model_beats_basic_perm(self):
        text = "给我出五道排列组合与概率的经典模型专项"
        self.assertEqual(resolve_tag(text), NUM_PERM_SPECIAL)
        ...
        self.assertIn("排列组合特殊模型", intent["batch_id"])
        self.assertEqual(
            suggest_batch_id(
                NUM_PERM_SPECIAL,
                today=date(2026, 9, 10),
                taken={"20260910_hermes_排列组合特殊模型_01"},
            ),
            "20260910_hermes_排列组合特殊模型_02",
        )
    ...
    def test_does_not_steal_named_perm_for_extreme(self):
        ...
        self.assertNotEqual(intent["tag"], NUM_EXTREME)

    def test_basic_perm_when_not_special(self):
        self.assertEqual(resolve_tag("出三道排列组合基础原理"), NUM_PERM_BASIC)
