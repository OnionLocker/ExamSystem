#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""02:00 weekday AI batch scheduler."""
...
def generation_prompt(run: dict, snapshot: dict, batch_dir: Path, db_path: Path | None = None) -> str:
    ...
    # 判断推理整卷为纯逻辑 20，不再由 Python 预置 5 道图形题
    if focus:
        ziliao_rule = (
            ...
            "Do not emit a mixed daily paper: no 判断+科推 mix, "
        ...
        ziliao_rule = (
            ...
            "Each material's Q5 must be 综合判断, with at least two different forms across materials "
            "(正确的是 / 无法推出 / 能推出几个 / 能推出). Keep tags[0] on the substantive skill, not 综合判断. "
        ...
        ziliao_rule = (
            ...
            "Output ALL 20 items as questions[], all 逻辑判断; no graphic items, no stem_images, no image_specs. "
            "Logic covers 加强/削弱/分析推理/结构相似/原因解释/归因; "
            "翻译推理 at most 2; never 定义判断, 类比推理, or 图形推理. "
            "Do NOT include 科学推理 in this batch. "
        ...
        ziliao_rule = (
            ...
            "Follow shuliang_pack slots in order. Questions 1-5: 数字推理; 6-15: 数学运算. "
            "Use each slot.tag; do not collapse all sequences to a generic 数字推理 tag. "
        ...
        ziliao_rule = (
            ...
            "Follow yanyu_pack slots in order. Questions 1-5 逻辑填空; then 片段阅读 and 语句表达. "
            "Use each slot.tag; do not write five 中心理解五种结构 in a row. "
            "Write questions.json in this paper order; do not shuffle."
        )
    else:
        ziliao_rule = (
    ...
    return (
        ...
        + (
            f"JSON question count must be exactly {run['planned_count']} (all 逻辑判断; no graphic items). "
            if daily_panduan
        ...
        "Science items MUST also emit figure_contract {kind,must_show,must_not,must_derive}; Python keeps it and draws from it. Do not omit must_show labels that appear in the stem.\n"
