...
排列组合与概率同理，按 4大计数 + 2大概率拆分（旧的合并标签 `特殊模型（八大情形与同组概率）` 与 `错位环形与同组概率` 只留给历史题）：

| 考法 | 识别信号 | 主标签 |
|---|---|---|
| 基础计数与网格路径 | 加乘原理、A/C公式、定序消序、网格最短路径避障 | `数量关系-逢考必有的排列组合与概率-基础原理与几何概型` |
| 相邻不相邻与位置限制 | 捆绑、插空、优限、双重限制反面剥离、地图染色 | `数量关系-逢考必有的排列组合与概率-相邻不相邻与位置限制` |
| 分堆分配与隔板模型 | 插板法（每份至少1/借还/预留）、平均分堆消序、定向分配 | `数量关系-逢考必有的排列组合与概率-分堆分配与定序消序` |
| 错位重排与环形圆桌 | 错位排序常数（0,1,2,9,44）、环形圆桌 (n-1)!、重复排列 | `数量关系-逢考必有的排列组合与概率-错位重排与环形圆桌` |
| 古典概型与定位秒杀 | 古典概型 m/n、对立事件 1-P、两人同组定位法秒杀、抽签原理 | `数量关系-逢考必有的排列组合与概率-古典概型与定位秒杀` |
| 分步独立与赛制决胜 | 独立重复试验（伯努利）、多轮闯关胜率、三局两胜/五局三胜提前终止 | `数量关系-逢考必有的排列组合与概率-分步独立与赛制决胜` |
