...
router.get('/', (_req, res) => {
  const items = db.prepare(`
    SELECT kaodian, module, subtype, attempts, correct, total_ms,
           last_seen, streak, note, mastery, mastery_note, updated_at
...
router.post('/mastery', (req, res) => {
  ...
  const score = Math.round(mastery);
  ...
  if (existing) {
    ...
    `).run({ kaodian, score, note, module, subtype });
  } else {
    ...
    db.prepare(`
      ...
      VALUES (@kaodian, @module, @subtype, 0, 0, 0, date('now'), 0, @note, @score, @note)
    `).run({
      kaodian,
      module: inferred,
      subtype,
